...
def crear_directorios():
    ...
    for carpeta in [os.path.join(base_path, 'small'), os.path.join(base_path, 'medium'), os.path.join(base_path, 'large')]:
...
def generar_instancias(max_turnos_dia=2, dispo_umbral=0, semilla=None):
    ...
    for tipo, dias_rng, trab_rng, turnos in zip(tipos, dias_rangos, trabajadores_rangos, turnos_rangos):
        for i in range(5):  # Genera 5 instancias por tipo
            ...
            carpeta = os.path.join(os.path.dirname(__file__), 'data', tipo)
            with open(os.path.join(carpeta, f"instancia_{id_instancia}.json"), "w", encoding="utf-8") as fjson:
                json.dump(instancia, fjson, ensure_ascii=False, indent=4)

            with open(os.path.join(carpeta, f"instancia_{id_instancia}.csv"), "w", newline='', encoding="utf-8") as fcsv:
...
if __name__ == '__main__':
    ...
    print("Instancias generadas en data/ con control de capacidad diaria y por turno.")
